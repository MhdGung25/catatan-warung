...
function ProductsPage() {
  ...
  return (
    <div className="p-4 md:p-10 min-h-screen bg-slate-50 dark:bg-slate-900 transition-all font-sans pb-40 md:pb-20">
      
      {/* HEADER */}
      <div className="mb-10 mt-6 md:mt-0 text-center md:text-left">
        ...
        <p className="text-sm md:text-base text-slate-500 dark:text-slate-400 font-medium italic">
          Kelola stok barang dagangan Anda di sini.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-10">
        ...
        {/* FORM INPUT */}
        <div className="lg:col-span-5 xl:col-span-4 bg-white dark:bg-slate-800 p-6 md:p-8 rounded-[2.5rem] shadow-xl border border-slate-100 dark:border-slate-700 h-fit lg:sticky lg:top-24 z-10">
          ...
          <form onSubmit={handleAddProduct} className="space-y-5">
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ref={codeInputRef}
                  type="text"
                  className="w-full pl-12 pr-4 py-4 rounded-2xl bg-slate-50 dark:bg-slate-900 dark:text-white font-bold outline-none border-2 border-transparent focus:border-indigo-500 transition-all shadow-inner"
                  value={newItem.code}
            ...
            <div>
              ...
              <div className="relative">
                <FiShoppingBag className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300" />
                <input
                  type="text"
                  className="w-full pl-12 pr-4 py-4 rounded-2xl bg-slate-50 dark:bg-slate-900 dark:text-white font-bold outline-none border-2 border-transparent focus:border-indigo-500 transition-all shadow-inner"
                  value={newItem.name}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <div className="relative">
                  <FiDollarSign className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300" />
                  <input
                    type="number"
                    className="w-full pl-10 pr-4 py-4 rounded-2xl bg-slate-50 dark:bg-slate-900 dark:text-white font-bold border-2 border-transparent focus:border-indigo-500 shadow-inner outline-none"
                    value={newItem.price}
            ...
            <button
              type="submit"
              className="w-full py-5 bg-indigo-600 text-white font-black rounded-3xl hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-200 dark:shadow-none flex items-center justify-center gap-3 mt-4 active:scale-95 tracking-widest text-sm"
            >
              SIMPAN KE GUDANG
            </button>
          </form>
        ...
        <div className="lg:col-span-7 xl:col-span-8 space-y-4">
          <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] shadow-xl border border-white dark:border-slate-700 overflow-hidden">
            ...
            <div className="overflow-x-auto">
              ...
              <table className="w-full text-left hidden md:table">
                <thead className="bg-slate-50/80 dark:bg-slate-900/80">
                  <tr>
                    ...
                    <th className="px-8 py-5 text-[10px] font-black uppercase text-slate-400 tracking-widest text-right">Hapus</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                  {products.length === 0 ? (
                    <tr>
                      <td colSpan="4" className="py-20 text-center text-slate-400 italic font-bold">Gudang masih kosong.</td>
                    </tr>
                  ) : (
                    products.map((p) => (
                      <tr key={p.id} className="hover:bg-slate-50 dark:hover:bg-slate-900/40 transition-colors">
                        <td className="px-8 py-6">
                          ...
                          <p className="text-xs text-slate-400 font-mono">ID: {p.code}</p>
                        </td>
                        ...
                        <td className="px-8 py-6 text-right">
                          <button onClick={() => handleDelete(p.id)} className="p-3 text-slate-300 hover:text-red-500 transition-all">
                            <FiTrash2 size={20} />
                          </button>
                        </td>
                      </tr>
              ...
              {/* Mobile Card View (Lebih nyaman di scroll) */}
              <div className="md:hidden divide-y divide-slate-100 dark:divide-slate-700">
                {products.length === 0 ? (
                  <div className="py-20 text-center text-slate-400 italic font-bold">Gudang Kosong.</div>
                ) : (
                  products.map((p) => (
                      ...
                      <div className="flex flex-col items-end gap-3">
                         <span className={`px-4 py-2 rounded-2xl font-black text-xs ${
                          ...
                          <button
                            ...
                            className="p-2.5 text-red-400 bg-red-50 dark:bg-red-900/20 rounded-xl"
                          >
                            <FiTrash2 size={18} />
                          </button>
                      </div>
          ...
          {/* Info bantuan tambahan di bawah list (opsional) */}
          <div className="flex items-center gap-3 p-6 bg-amber-50 dark:bg-amber-900/10 rounded-[2rem] border border-amber-100 dark:border-amber-900/20">
            <FiAlertCircle className="text-amber-500 shrink-0" size={20} />
            <p className="text-[10px] md:text-xs text-amber-700 dark:text-amber-500 font-bold uppercase tracking-wider leading-relaxed">
              Tips: Masukkan kode barcode yang sama untuk menambah stok barang yang sudah ada secara otomatis.
            </p>
          </div>
        </div>
